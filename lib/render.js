	...
	this._renderImage_geLabels = function (entry) {
		...
		return entry.mapRegions
			.map(region => {
				...
				polylabel(mapRegionPointsGeoJson);
...
Renderer.hover = class {
	...
	static getShowWindow (eleContent, position, opts) {
		...
		if (opts.pageUrl && !position.window._IS_POPOUT && !Renderer.get().isInternalLinksDisabled()) {
			ee`<a class="ve-hwin__top-border-icon glyphicon glyphicon-modal-window" title="Go to Page" href="${opts.pageUrl}"></a>`
				.appendTo(brdEleTopRhs);
		}

		if (!position.window._IS_POPOUT && !opts.isPopout) {
			ee`<span class="ve-hwin__top-border-icon glyphicon glyphicon-new-window hvr__popout" title="Open as Popup Window"></span>`
				.onn("click", evt => {
					evt.stopPropagation();
		...
		ee`<span class="ve-hwin__top-border-icon glyphicon glyphicon-remove" title="Close (CTRL to Close All)"></span>`
			.onn("click", (evt) => {
				evt.stopPropagation();

function baseCharacterIo () {
	...
	const getImportEntry = (payload) => {
		if (payload?.character) return payload.character;
		if (Array.isArray(payload?.characters) && payload.characters.length) {
			if (payload.characters.length === 1) return payload.characters[0];
			const options = payload.characters.map((entry, i) => `${i + 1}: ${entry.attributes?.name || "Unnamed character"}`).join("\n");
	...
	const applyCharacterImport = (character, entry) => {
		...
		if (character.view?.updateSheetValues) character.view.updateSheetValues();
	};
			...
			.on("click", ".character-json-import", (event) => {
				...
				$input.on("change", () => {
					...
					reader.onload = () => {
						...
						const name = entry.attributes?.name || character.get("name");
